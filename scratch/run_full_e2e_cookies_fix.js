...
const historyDir = path.join(process.cwd(), 'docs', 'historico', '2026-09-14_14h05_teste-real-e2e-pos-correcao-cookies');
...
(async () => {
  log('=== INICIANDO TESTE REAL PONTA A PONTA PÓS-CORREÇÃO DE COOKIES LGPD (#botao-aceitar-todos) ===');
  log(`Pasta de histórico criada: ${historyDir}`);

  // 1. Salvar código utilizado (handler de cookies)
  const cookieBannerSrc = path.join(process.cwd(), 'lib', 'services', 'automacao', 'cookieBanner.ts');
  const cookieBannerDest = path.join(scriptsDir, 'cookieBanner.ts');
  fs.copyFileSync(cookieBannerSrc, cookieBannerDest);
  log('✓ Código do handler cookieBanner.ts copiado para scripts/');
  ...
  try {
    ...
    log('Passo 4: Digitando item real no Rascunho: "Cabo Flex 2.5mm" (Quantidade 1)...');
    ...
    log('Passo 10: Aguardando robô RPA processar cotação no Cicalfer e capturar carrinho real...');
    ...
    log('Acessando carrinho do Cicalfer pós-login B2B para capturar Print 02...');
    ...
    log('=== TESTE REAL PONTA A PONTA COM HANDLER DE COOKIES CONCLUÍDO COM SUCESSO ===');
  } catch (err) {
